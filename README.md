...
- `--impl-backend`, `--fix-backend`, `--reviewer-backend` pin individual phases to a different backend — e.g. `--backend claude-code --reviewer-backend codex` runs Claude Code as the implementer/fixer with Codex as a fresh-eyes reviewer. Matching `--impl-cmd`, `--fix-cmd`, `--reviewer-cmd` override the per-phase CLI binary. The matching env vars `AGENT_IMPL_BACKEND` / `AGENT_FIX_BACKEND` / `AGENT_REVIEWER_BACKEND` (and `AGENT_*_CMD`) are honoured by `ai run` so the same shell config produces the same mixed run on every invocation. The bare `ai` TUI's "new run" verb takes a different tack: its third step (after prompt and workspace) is an opinionated backend picker with two presets — **Cursor** (Opus implementer + GPT reviewer) and **Claude Code + Codex** (Claude implementer/fixer + Codex reviewer). Both presets are pinned: a stale `AGENT_CMD` / `AGENT_*_BACKEND` / `AGENT_*_CMD` in the surrounding shell will not silently rewrite them, so the layout shown in the modal is exactly what runs. Operators who want env-driven backend resolution should use `ai run` from the shell.
