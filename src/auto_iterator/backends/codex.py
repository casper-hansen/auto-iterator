...
Event shapes consumed here (observed against codex-cli 0.124.x):

* ``{"type": "thread.started", "thread_id": "..."}`` — session id, ignored.
* ``{"type": "turn.started"}`` — turn begins, ignored.
* ``{"type": "item.started", "item": {...}}`` — a tool call or message
  begins. Items with a tool-like ``type`` (``command_execution``,
  ``file_change``, ...) mark the start of a tool call (pending++).
* ``{"type": "item.completed", "item": {...}}`` — an item finishes. For
  tool items, pending--. For ``{"type": "agent_message", "text": ...}``
  items, accumulate the text as authoritative final output (Codex does
  not emit incremental text deltas — the whole message arrives here).
* ``{"type": "turn.completed", "usage": {...}}`` — the final event; used
  as the ``saw_result`` signal.
"""
...
class CodexBackend:
    ...
    # Codex only runs OpenAI-family models; ``gpt-5.4`` is the current
    # flagship. Effort is pinned to ``xhigh`` via ``_BASE_FLAGS`` below —
    # the Codex CLI doesn't expose a dedicated ``--effort`` flag, so we
    # route it through ``-c model_reasoning_effort="xhigh"`` to mirror what
    ...
    def build_continue_cmd(
        ...
        # ``codex exec resume --last`` picks the most recent session in the
        # current cwd (``--all`` would disable that filter). Since the
        # subprocess is launched with ``cwd=workspace``, the correct session
        # is selected without having to track thread_ids.
        cmd = [
            agent_cmd, "exec", "resume", "--last",
            *self._BASE_FLAGS,
            "--model", model,
        ]
        cmd.extend(extra_flags)
